...
import gestion_cahiers_textes.pages.chefdepartement.ListeResponsableClasse;
import gestion_cahiers_textes.pages.chefdepartement.ListeEnseignantPage;
import gestion_cahiers_textes.pages.chefdepartement.EnregistrerUtilisateurPage;
import javax.swing.*;
import java.awt.*;
import java.awt.event.ActionListener;

public class ChefDepartementPage extends JFrame {

    public ChefDepartementPage() {
        setupFrame();
        add(createHeader());
        add(createLeftPanel());
    ...
    private void setupFrame() {
        setTitle("Page d'accueil - Chef de Département");
        setDefaultCloseOperation(EXIT_ON_CLOSE);
        setSize(1200, 700);
        setLocationRelativeTo(null);
        setLayout(null);
        getContentPane().setBackground(new Color(245, 245, 245)); // Ajout d'un fond clair global
    }

    private JPanel createHeader() {
        JPanel header = new JPanel(new BorderLayout());
        header.setBounds(0, 0, 1200, 80);
        header.setBackground(new Color(34, 45, 56)); // Changement de couleur du header
    ...
    private JPanel createNavPanel() {
        ...
        String[][] navItems = {
            {"Liste des responsables ", "liste des responsables"},
            {"Ajouter un utlisateur", "ajouter utilisateur"},
            {"Liste des enseignants", "liste des enseignants"},
            {"Voir les cours", "voir cours"},
    ...
    private JPanel createLeftPanel() {
        ...
        addGridButton(buttonGrid, "Liste des enseignants", e -> {
            new ListeEnseignantPage().setVisible(true);
            dispose();
        });
        addGridButton(buttonGrid, "Liste des responsables", e -> {
            new ListeResponsableClasse().setVisible(true);
            dispose();
        });
        addGridButton(buttonGrid, "Ajouter un utilisateur", e -> {
    ...
    private JButton createNavButton(String text, String link) {
        ...
        button.addActionListener(e -> {
            ...
            if("liste des responsables".equals(link)){
                new ListeResponsableClasse().setVisible(true);
            }
             if("ajouter utilisateur".equals(link)){
                new EnregistrerUtilisateurPage().setVisible(true);
            }
             if("liste des enseignants".equals(link)){
                new ListeEnseignantPage().setVisible(true);
            }
        });
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(ChefDepartementPage::new);
    }
}
